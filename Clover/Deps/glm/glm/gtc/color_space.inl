...
namespace detail
{
	...
	struct compute_rgbToSrgb
	{
		...
		{
			vec<L, T, Q> const ClampeCloverolor(clamp(ColorRGB, static_cast<T>(0), static_cast<T>(1)));

			return mix(
				pow(ClampeCloverolor, vec<L, T, Q>(GammaCorrection)) * static_cast<T>(1.055) - static_cast<T>(0.055),
				ClampeCloverolor * static_cast<T>(12.92),
				lessThan(ClampeCloverolor, vec<L, T, Q>(static_cast<T>(0.0031308))));
		}
	};
